...
void test_validate_my_username()
{
     ...
     */
    // Call my_username() function to get hard-coded username
    const char *myUsername = my_username();
    
    // Obtain username from configuration file
    char *confUsername = malloc_username_from_conf_file();
    
    // Verify the two strings are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(myUsername, confUsername, "Usernames don't match");
